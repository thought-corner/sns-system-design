...
- 순서는 게시글 id 내림차순 (작성 순). 삭제된 글, 취소된 리포스트, 원본이 삭제된 리포스트 행, **내가 조회 기록 (`POST …/views`)을 남긴 글** (조회 기록은 원본 기준이므로 그 원본의 리포스트 행도 같이 빠진다), 작성 후 30일이 지난 글은 나오지 않는다
  (코드 상수 `TimelinePolicy`). 인기 글(좋아요 10개 이상, 팔로우 무관)을 섞는 소스는 기본 꺼져 있다(`TimelinePolicy.POPULAR_SOURCE_ENABLED`).
...
- 타임라인은 Redis 에 게시글 id 만 두는 파생 캐시다 (`sns:timeline:*`, 사용자당 최근 800개, 7일 미접속 시 소멸 — 소멸한 뒤엔 다음 조회가 다시 만들 때까지 새 글이 쌓이지 않는다). 없으면 요청 안에서 PostgreSQL 로 다시 만들며, Redis
  장애 시에도 응답은 성공한다 (느려질 뿐). 타임라인 값(상한·TTL·임계값·토글)은 환경 변수가 아니라 코드 상수 `TimelinePolicy` 다 — 바꾸면 이 문단도 같이 바뀐다.
