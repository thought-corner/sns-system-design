...
- [docs/서버 세션과 Redis 중앙 저장소 설계.md](docs/서버%20세션과%20Redis%20중앙%20저장소%20설계.md) — 로그인 · 세션 · 동시 로그인 제어 · CSRF
- [docs/팔로우 시스템 설계.md](docs/팔로우%20시스템%20설계.md) — 멱등 관계 행 · 팔로우 통계 행 · 소프트 삭제
- [docs/게시글 설계.md](docs/게시글%20설계.md) — 통합형 `posts` 행 · 답글·인용·리포스트·좋아요·조회수 · 게시글 통계 행
- [docs/미디어 설계.md](docs/미디어%20설계.md) — presigned PUT 직접 업로드 · 완료 검증 · 게시글 첨부 · presigned GET
- [docs/타임라인 설계.md](docs/타임라인%20설계.md) — Redis 팬아웃 · 대형 계정 읽기 병합 · 재구축 · 후보 수집 → hydrate → 필터
...
홈 타임라인 — 내가 팔로우하는 사람들과 나의 게시글 (일반·인용·리포스트 행, **답글 제외**)을 최신순으로. 인증된 세션이 필요하다.

| 메서드 | 경로            | 설명                                |
|--------|-----------------|-------------------------------------|
| `GET`  | `/api/timeline` | 홈 타임라인 조회(커서 페이지네이션) |

- 쿼리 `cursor`(선택 — 이 게시글 id 보다 오래된 것부터), `limit`(기본 20, 최대 50 — 넘으면 50 으로 잘린다). 응답
  `{items[]{post, original}, nextCursor}`.
  `post` 는 게시글 응답과 같은 모양이고, `post` 가 리포스트 행이면 `original` 에 원본 게시글 응답이 함께 실린다 (아니면 `null`). `nextCursor` 가 `null` 이면
  끝이다 — 항목이 `limit`
  보다 적어도 `nextCursor` 가 있으면 더 있을 수 있다 (삭제된 글은 응답에서 빠진다).
- 순서는 게시글 id 내림차순 (작성 순). 삭제된 글, 취소된 리포스트, 원본이 삭제된 리포스트 행, **내가 조회 기록 (`POST …/views`)을 남긴 글** (조회 기록은 원본 기준이므로 그 원본의
  리포스트 행도 같이 빠진다), 작성 후 30일이 지난 글은 나오지 않는다 (코드 상수 `TimelinePolicy`). 인기 글 (좋아요 10개 이상, 팔로우 무관)을 섞는 소스는 기본 꺼져 있다
  (`TimelinePolicy.POPULAR_SOURCE_ENABLED`).
- 게시글이 작성·인용·리포스트되면 **커밋 뒤 비동기로** 팔로워들의 타임라인에 반영된다 (수 초 지연 가능). 팔로우하면 상대의 최근 글 50개가 바로 들어오고, 언팔로우하면 상대의 글이 빠진다. 팔로워가
  10,000명 이상인 계정의 글은 팔로워 타임라인에 복사되지 않고 읽을 때 병합된다 (결과는 같다).
- 타임라인은 Redis 에 게시글 id 만 두는 파생 캐시다 (`sns:timeline:*`, 사용자당 최근 800개, 7일 미접속 시 소멸 — 소멸한 뒤엔 다음 조회가 다시 만들 때까지 새 글이 쌓이지 않는다).
  없으면 요청 안에서 PostgreSQL 로 다시 만들며, Redis 장애 시에도 응답은 성공한다 (느려질 뿐). 타임라인 값 (상한·TTL·임계값·토글)은 환경 변수가 아니라 코드 상수
  `TimelinePolicy` 다 — 바꾸면 이 문단도 같이 바뀐다.
